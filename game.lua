Modules = {
	...
	bundle = "bundle",
	ease = "ease",
	explode = "github.com/aduermael/modzh/explode:786f55e",
	particles = "particles",
	sfx = "sfx",
	uitheme = "uitheme",
	ui = "uikit",
	fifo = "github.com/aduermael/modzh/fifo:786f55e",
	niceLeaderboard = "github.com/aduermael/modzh/niceleaderboard:786f55e",
...
spawners = {
	...
	spawnEnnemy = function(room, position, _ennemyType)
		...
		npc.kill = function(reason)
			...
			dustifyModule.dustify(npc, {
				direction = npc.Motion,
				velocity = Number3(50, 100, 5),
				bounciness = 0.15,
	...
	spawnGroundParticle = function(position, color)
		...
		pcube.LocalRotation = { math.random() * math.pi * 2.0, math.random() * math.pi * 2.0, 0.0 }
	...
	increaseStat = function(stat, amount, _obj)
		...
		if Client.BuildNumber >= 230 and badge then
			if stat == "coins" and gameManager._stats.coins >= 100 then
				badge:unlockBadge("greedydiver")
			elseif stat == "destroyedProps" and gameManager._stats.destroyedProps >= 50 then
				badge:unlockBadge("breakitall")
			end
		end
	end,
	startGame = function()
...
uiManager = {
	...
	showLeaderboardScreen = function(totalScore, floorReached)
		...
		leaderboard:setParent(frame)
		leaderboard.Width = frame.Width - uiPadding * 2
		leaderboard.Height = frame.Height - newGameButton.Height - uiPadding * 3
		leaderboard.LocalPosition = { frame.Width / 2 - leaderboard.Width / 2, uiPadding + newGameButton.Height + uiPadding }
		leaderboard:show()
	end,
